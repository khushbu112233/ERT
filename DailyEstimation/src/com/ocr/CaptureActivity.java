...
import com.ocr.language.LanguageCodeHelper;
import com.ocr.language.TranslateAsyncTask;
...
public final class CaptureActivity extends Activity implements
	...
	private String targetLanguageReadable; // Language name, for example,
											// "English"
	private int pageSegmentationMode = TessBaseAPI.PageSegMode.PSM_OSD_ONLY;
	...
	boolean handleOcrDecode(final OcrResult ocrResult) {
		...
		Log.e("LastBitmap:", "w: " + lastBitmap.getWidth() + " h: "
		...
		// lastBitmap.compress(CompressFormat.PNG, 0 /* ignored for PNG */,
		// bos);
			...
			new TranslateAsyncTask(this, sourceLanguageCodeTranslation,
					...
					new IResultListner() {
						...
						public void result(Object result, boolean isSuccess) {
							if (isSuccess) {
								ocrResult.setText((String) result);
							}
	...
	void handleOcrContinuousDecode(OcrResult ocrResult) {
		 ...
		 * rendering viewfinderView.addResultText(new
		 * OcrResultText(ocrResult.getText(), ocrResult.getWordConfidences(),
		 * ocrResult.getMeanConfidence(), ocrResult.getBitmapDimensions(),
		 * ocrResult.getRegionBoundingBoxes(),
		 * ocrResult.getTextlineBoundingBoxes(),
		 * ocrResult.getStripBoundingBoxes(), ocrResult.getWordBoundingBoxes(),
		 ...
		 * statusViewTop.setBackgroundResource
		 * (R.color.status_top_text_background);
		 *
	...
	void handleOcrContinuousDecode(OcrResultFailure obj) {
		 ...
		 *
		 * if (CONTINUOUS_DISPLAY_METADATA) { // Color text delimited by '-' as
		 * red. statusViewBottom.setTextSize(14); CharSequence cs =
		 * setSpanBetweenTokens("OCR: " + sourceLanguageReadable +
	...
	private void resetStatusView() {
		resultView.setVisibility(View.GONE);
		/*
		 * if (CONTINUOUS_DISPLAY_METADATA) { statusViewBottom.setText("");
		 * statusViewBottom.setTextSize(14);
		 * statusViewBottom.setTextColor(getResources
		 * ().getColor(R.color.status_text));
		 * statusViewBottom.setVisibility(View.VISIBLE); } if
		 * (CONTINUOUS_DISPLAY_RECOGNIZED_TEXT) { statusViewTop.setText("");
	...
	void setStatusViewForContinuous() {
		viewfinderView.removeResultText();
		/*
		 * if (CONTINUOUS_DISPLAY_METADATA) { statusViewBottom.setText("OCR: " +
		 * sourceLanguageReadable + " - waiting for OCR..."); }
		 */
	}
	...
	// * run. The easiest way to do this is to check android:versionCode from
	// the manifest, and compare
	...
	//
	// // Record the last version for which we last displayed the What's New
	// (Help) page
	...
	private void onGetResult(OcrResult ocrResult) {
		Intent data = new Intent();

		lastBitmap = getResizedBitmap(lastBitmap, 640, 640);

		Global.setImageBitmap(lastBitmap);
		data.putExtra("text", ocrResult.getText());
