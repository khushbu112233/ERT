...
final class OcrRecognizeAsyncTask extends AsyncTask<Void, Void, Boolean> {
	...
	OcrRecognizeAsyncTask(CaptureActivity activity, TessBaseAPI baseApi,
		...
		// Log.e("#OCR Image Before#", "::W::"+width+"  ::H::"+height);
		//
		...
		// height = screenWidth;
		//
		// Log.e("#OCR Image After#", "W="+width+"::H::"+height);
	...
	protected Boolean doInBackground(Void... arg0) {
		...
		Log.e("#OCR Image#", "W=" + width + "::H::" + height);

		activity.getCameraManager().adjustFramingRect(width, height);
		...
		try {
			...
			Log.e("Ocr recognized Text", "textResult >>" + textResult);
	...
	public Bitmap fixOrientation(Bitmap mBitmap) {
		...
		if (mBitmap != null && (mBitmap.getWidth() > mBitmap.getHeight())) {
			...
			Log.e("OcrRecognize", "Bitmap rotated");
		} else
			Log.e("OcrRecognize", "Bitmap not required to rotate");
